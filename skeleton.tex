...
Mixes to yield worse anonymity. We justify our hypothesis, assuming that each
...
Our initial hypothesis believed that by increasing the failure probability; we
would see a decline in anonymity because a considerable amount of Mixes would
receive less traffic. This hypothesis is invalidated since anonymity seems to
stay the same for different failure probabilities. According to Figure ..., for
a 20\% crash probability, we get slightly higher entropy, assuming 100 clients in
the network. Nonetheless, once we scale up the user base and crash
probabilities, the anonymity ranges in equivalent levels. Besides that, our
initial assumption for higher latency is false since, as we notice in Figure
...
more detail, after skimming the log files, we observe that invalid paths occur
sporadically; hence the increased latency of a message does not affect the
average network latency. Furthermore, anonymity is not affected because entropy
...
fascinating. Therefore, a similar idea arose to investigate statically
imbalanced Stratified networks. Our perception of "static" describes a formation
where each layer has a fixed number of Mixes, yet this number is different. In
particular, we seek to study the effect on anonymity when we (a) evenly increase
the number of Mixes per layer and (b) have more or fewer Mixes on the first,
middle and last layer of a Stratified Mixnet.
...
variable number of Mixes in the middle layer, testing three, five and eight
Mixes setups. Next, we repeat the same trial starting with 1 Mix and steadily
increasing this number to 5 Mixes per layer for the first and last layer,
respectively. 
...
continuously from senders to Mixes and from Mixes to receipients. Nonetheless,
something different occurs assuming a smaller user base. Notably, a 3 Mix per
layer configuration yields slightly lower entropy compared to 4 Mix layers.
Likewise, a 4 Mix per layer network has slightly lower entropy than a 5 Mix per
layer design. 
...
Furthermore, we observe latency at a constant level of 0.3. The exact value was
